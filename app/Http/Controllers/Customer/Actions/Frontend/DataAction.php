...
class DataAction{
    ...
    public function getHotAndNew(){
        $finalData = [];
        $movies = Movie::select(['id','title', 'slug', 'thumbnail', 'type'])->orderBy('id', 'DESC')->limit(10)->get()->map(function($movie){
        ...
        $trendingMovies = Movie::select(['id','title', 'slug', 'thumbnail', 'type'])->orderBy('rating', 'DESC')->limit(10)->get()->map(function($movie){
            $premiumStatus = (new PremiumContentActionUnAuthorized($movie))->checkPremium()['status'] ?? false;
            $movie['premium_status'] = $premiumStatus;
            return $movie;
        });

        $tvSeries = TvSeries::select(['id','title', 'slug', 'thumbnail', 'type'])->orderBy('id', 'DESC')->limit(10)->get()->map(function($movie){
            $premiumStatus = (new PremiumContentActionUnAuthorized($movie))->checkPremium()['status'] ?? false;
            $movie['premium_status'] = $premiumStatus;
            return $movie;
        });
        ...
        $trendingtvSeries = TvSeries::select(['id','title', 'slug', 'thumbnail', 'type'])->orderBy('rating', 'DESC')->limit(10)->get()->map(function($movie){
            $premiumStatus = (new PremiumContentActionUnAuthorized($movie))->checkPremium()['status'] ?? false;
            $movie['premium_status'] = $premiumStatus;
            return $movie;
        });

        $webSeries = WebSeries::select(['id','title', 'slug', 'thumbnail', 'type'])->orderBy('id', 'DESC')->limit(10)->get()->map(function($movie){
            $premiumStatus = (new PremiumContentActionUnAuthorized($movie))->checkPremium()['status'] ?? false;
            $movie['premium_status'] = $premiumStatus;
            return $movie;
        });
