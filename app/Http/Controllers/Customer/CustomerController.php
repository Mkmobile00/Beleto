...
class CustomerController extends Controller
{
    ...
    public function customerProfile(Request $request)
    {
        ...
        if($customer->customerDetail){
            if ($customer->customerDetail->photo_status != 'social') {
                if($customer->customerDetail->photo_from=='web'){
                    $customer->setAttribute('photo', $customer->customerDetail->photo ?? null);
                }else{
            ...
            $customer->setAttribute('photo',null);
        }
    ...
    public function socialLoginOrRegister(Request $request)
    {
        ...
        try {
            ...
            if (!$customerDetails) {
                ...
                $customerDetails['photo']=$request->photo ?? $customerDetails->photo;
                $customerDetails['photo_status']='social';
    ...
    public function customerNewLogin(Request $request){
        $validator = Validator::make($request->all(), [
        ...
        try {
            ...
            $customer = Customer::where('email', $request->email)->withTrashed()->first();
            if($customer->status->value=='2' || $customer->status->value=='3'){
                $status=$customer->status->value=='2' ? 'Inactive':'Blocked';
                $response = [
                    'error' => true,
                    'data' => null,
                    'msg' => "Sorry Your Account Has Been ${status} Plz Contact Kantipur Cinemas."
                ];
                return response()->json($response, 200);
            }
            if($customer->from_old){
                $response = [
                    'error' => false,
                    'resetPassword'=>true,
                    'data' => null,
                    'msg' => 'Plz Reset Your Password !!'
                ];
                return response()->json($response, 200);
            }
            if (!$customer) {
            ...
            //                 'error' => true,
            //                 'data' => null,
            //                 'msg' => 'Device Limit Is Full Please Logout From Another Device To Login This Device !!'
            //             ];
            //             return response()->json($response, 200);
            //         }
            //     }else{
            ...
            if($customer->manual_subscription){
                ...
                if((new CustomerAddDevice($customer))->setDeviceList($request)){
                    if ($subscriptionStatus) {
                        $alreadyAddedToList = CustomerDeviceList::where('customer_id',$customer->id)->where('device_serial_num', $request->device_serial_num)->withTrashed()->first();
                        if(!$alreadyAddedToList){
                            $setDeviceStatus=(new CustomerAddDevice($customer))->setDeviceList($request);
                    ...
                    $alreadyAddedToList = CustomerDeviceList::where('customer_id',$customer->id)->where('device_serial_num', $request->device_serial_num)->first();
                    if(!$alreadyAddedToList){
                        $response = [
                                'error' => true,
                                'data' => null,
            ...
            return $this->responseApiError('Sorry !! These Credential Doesnot Match Our Records !!', null, 200);
        }
    }
}
