...
class NepaliCalendar
{
	...
	/**
	 * Get the last day month of the year in nepali
	 *
	 */
	public function get_nep_last_day_of_month_year(int $year, int $month): int
	{
		// dd($year, $month);
		return collect($this->_bs)->filter(function ($row) use ($year) {
			return $row[0] == (int) $year;
		})->flatten()->toArray()[$month] ?? 0;
	}
	...
	//get the yearly lastDayofMonth and DayofWeek of the year and month
	function getNepaliYearlyLastDayOfMonthAndWeekday(int $year)
	{
		$dataLastDayOfMonth = [];
		$dataDayOfWeek = [];
		for ($i = 1; $i <= 12; $i++) {
			$lastDayOfMonth = $this->get_nep_last_day_of_month_year($year, $i);

			$dataLastDayOfMonth[$i] = $lastDayOfMonth;

			for ($j = 1; $j <= $lastDayOfMonth; $j++) {
				if ($j >= 32)
					continue;
				$dataDayOfWeek[$i][$j] = Carbon::parse($year . '-' . $i . '-' . $j)->dayOfWeek;
			}
		}

		return [
			'lastDayOfMonth' => $dataLastDayOfMonth,
			'dayOfWeek' => $dataDayOfWeek
		];
	}
}

function languageData($lang = null, $attr = null)
{
...
function uploadImage($image, $thumb = "50x50")
{
	list($thumb_width, $thumb_height) = explode("x", $thumb);
    $path = public_path() . '/storage/photos/1/visitor/';
    if (!File::exists($path)) {
        File::makeDirectory($path, 0777, true, true);
    }
    $image_name =date("YmdHis") . "-" . rand(0, 9999) . "." . $image->getClientOriginalExtension();
    ...
    if ($status) {
		...
		return env('APP_URL').'/storage/photos/1/visitor/'.$image_name;
    } else {
        return null;
    }
}

function uploadProfileImage($image, $dir, $thumb = "50x50")
{
    list($thumb_width, $thumb_height) = explode("x", $thumb);
...
function calculateAge($engDateOfBirth)
{
    // Convert the English date of birth to a Carbon instance
    $dob = Carbon::parse($engDateOfBirth);
...
function yesNo(){
	$dataInput='';
	$dataInput.='<select class="form-control" name="">';
	$dataInput.='<option value="yes">Yes</option>';
	$dataInput.='<option value="no">no</option>';
	$dataInput.='</select>';
	return $dataInput;
}

function video(){
	$videoQuality=VideoQuality::where('status','1')->orderBy('id','desc')->get();
	$dataInput='';
	$dataInput.='<select class="form-control" name="">';
	foreach($videoQuality as $quality){
		$dataInput.='<option value="'.$quality->quality.'">'.$quality->quality.'</option>';
	}
	$dataInput.='</select>';
	return $dataInput;
}
...
function number(){
	...
	$dataInput='';
	$dataInput.='<input class="form-control" min="1" name="" type="number"></input>';
	return $dataInput;
}
...
function audio(){
	...
	$dataInput.='<select class="form-control" name="">';
	foreach($videoQuality as $quality){
		$dataInput.='<option value="yes">'.$quality->quality.'</option>';
	}
	$dataInput.='</select>';
	return $dataInput;
}
...
function videoQuality($id){
	$device=VideoQuality::where('id',$id)->first();
	if($device){
...
function audioQuality($id){
	$device=AudioQuality::where('id',$id)->first();
	if($device){
		return $device->quality;
	}else{
		return '';
...
function checkFav($customerId,$movieId,$type){
	$status=false;
	$data=CustomerWishList::where('customer_id',$customerId)->where('movie_id',$movieId)->where('video_type',$type)->first();
	if($data){
		$status=true;
	}
	return $status;
...
function checkLike($customerId,$movieCode){
	$data=VideoLike::where('video_unique_code',$movieCode)->where('customer_id',$customerId)->first();
	if($data){
		return $data->status->value=='1' ? true : false;
	}
	return false;
}

function checkDislike($customerId,$movieCode){
	$data=VideoLike::where('video_unique_code',$movieCode)->where('customer_id',$customerId)->first();
	if($data){
		return $data->status->value=='2' ? true : false;
	}
	return false;
}


function checkCustomerPreimumContentStatus($customer,$premiumDetails,$movieId){
	$premiumContent=PremiumContent::where('id',$premiumDetails['id'])->where('movie_id',$movieId)->first();
	$customerPayment=PremiumContentPayment::where('premium_content_id',$premiumContent->id)->where('customer_id',$customer->id)->first();
	if($customerPayment){
		return true;
	}
	else{
...
function extractYoutubeCode($url)
{
	...
	function generateSinganture($message)
    {
        try {
            $hmac = hash_hmac('sha512', $message, true);
...
function filterPrice($amount,$currency_type){
	$currencyType=Currency::where('id',$currency_type)->first();
        if(!$currencyType){
            $response = [
                'error' => true,
                'data' => null,
        ...
        $amount=$amount*$currencyType->value;
        return (int)$amount;
}

function checkCustomerSusbcription($customer){
	$customerHasSubscription=false;
	$customerSubscription=$customer->subscription->where('is_expired','0');
        if(count($customerSubscription) > 0){
            $customerHasSubscription=true;
        }else{
            $customerHasSubscription=false;
        }
        return $customerHasSubscription;
}
...
function signHMAC($message, $secret) {
        ...
        $digest = hash_hmac('sha512', $message, $secret, true);
        $encodedDigest = base64_encode($digest);
        return $encodedDigest;
   
}

function signHMAC1($message) {
    
	$digest = hash_hmac('sha512', $message, true);
	$encodedDigest = base64_encode($digest);
	return $encodedDigest;

}

function userWishList($customer){
	$customerWishList = $customer->wishList;
            $moviesList = [];
            foreach ($customerWishList as $index => $list) {
                switch ($list->video_type) {
                    case 1: //movie
                        $moviesList[]=wishListMovie($list);
                        break;
                    case 2: //tvshow
                        $moviesList[] = wishListTvSeries($list);
                        break;
                    case 3: // webseries
                        $moviesList[] =  wishListWebSeries($list);
                        break;
                    default:
                        return null;
                        break;
	...
	function wishListMovie($data)
		{
			return [
				'id' => $data->id,
				'title' => $data->movie->title ?? null,
				'slug' => $data->movie->slug ?? null,
				'image' => $data->movie->thumbnail ?? null,
				'summary' => $data->movie->summary ?? null,
				'rating' => $data->movie->rating ?? null,
				'movie_id' => $data->movie->id ?? null,
				'type' => $data->video_type ?? null,
				'release_date' => $data->movie->release_date ?? null,
				'country' => $data->movie->country ? $data->movie->country->map(function ($item) {return $item->title ?? null; }) : null
			
			];
		}

    function wishListTvSeries($data)
		{
			
			return [
				'id' => $data->id,
				'title' => $data->tvSeries->title,
				'slug' => $data->tvSeries->slug,
				'image' => $data->tvSeries->poster,
				'summary' => $data->tvSeries->summary,
				'rating' => $data->tvSeries->rating,
				'movie_id' => $data->tvSeries->id,
				'type' => $data->video_type,
				'release_date' => $data->tvSeries->release_date ?? null,
				'country' => $data->tvSeries->country->map(function ($item) {
					return $item->title;
				})
			];
		}

    function wishListWebSeries($data)
		{
			return [
				'id' => $data->id,
				'title' => $data->webSeries->title,
				'slug' => $data->webSeries->slug,
				'image' => $data->webSeries->poster,
				'summary' => $data->webSeries->summary,
				'rating' => $data->webSeries->rating,
				'movie_id' => $data->webSeries->id,
				'type' => $data->video_type,
				'release_date' => $data->webSeries->release_date ?? null,
				'country' => $data->webSeries->country->map(function ($item) {
					return $item->title;
				})
			];
		}

	function checikUnique($email){
