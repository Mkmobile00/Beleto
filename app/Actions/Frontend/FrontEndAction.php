...
class FrontEndAction{
    ...
    public function getHomePageData(){
        ...
        $featuredSection = FeaturedSection::where('status', VideoEnum::ACTIVE)->get()->map(function ($item) {
            switch ($item->type) {
                ...
                case FeaturedSectionTypeEnum::TVSERIES:
                    $dataItems = collect($item->items)->pluck('item_id');
                    $movies = TvSeries::whereIn('id', $dataItems)->with('genre')->select(['id','title', 'slug', 'poster', 'type','thumbnail','run_time'])->where('publication','1')->orderBy('id', 'DESC')->take(10)->get()->toArray();
                    break;
                case FeaturedSectionTypeEnum::WEBSERIES:
                    $dataItems = collect($item->items)->pluck('item_id');
                    $movies = WebSeries::whereIn('id', $dataItems)->with('genre')->select(['id','title', 'slug', 'poster', 'type','thumbnail','run_time','position'])->where('publication','1')->orderBy('position','ASC')->take(10)->get()->toArray();
                    break;
                case FeaturedSectionTypeEnum::MOVIES:
                    $dataItems = collect($item->items)->pluck('item_id');
                    $movies = Movie::whereIn('id', $dataItems)->with('genre')->select(['id','title', 'slug', 'poster', 'type','thumbnail','run_time','position'])->where('publication','1')->orderBy('position','ASC')->take(10)->get()->toArray();
                    break;
                default:
                   return null;
                    break;
        ...
        $data['finalData'] = array_merge($featuredSectionArray, $finalData);
        $data['hotAndNew']=(new DataAction())->getHotAndNew();
        $data['stremmings']=Stremming::where('publication','1')->orderBy('position','ASC')->get();
        return $data;
    }
}