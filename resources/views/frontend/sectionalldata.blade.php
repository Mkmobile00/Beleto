...
@section('contents')
                ...
                @foreach ($sliders as $slider)
                    <div class="swiper-slide">
                        @if ($slider->type == 'video')
                            <video id="my-video" class="video-js play_video" controls preload="auto" poster=""
                                data-setup="{}" autoplay muted>
                                <source src="{{ route('video.playlist', $slider->transcode . '.m383') }}"
                        ...
                        @if ($slider->movie_id != null)
                            <div class="video_play_list_btn">
                                <a href="{{ route('customer.addWishList', [$slider->movie_id, $slider->item_type]) }}"
                                    class="{{ $slider->is_wish ? 'wishlistActive' : '' }}">{{ $slider->is_wish ? 'Remove From My List' : 'Add to My List' }}
    ...
    <section id="movies" class="all_movies">
                ...
                <div class="d-flex flex-wrap justify-content-center" style="gap: 20px">
                    @if (count($movies) > 0)
                        @foreach ($movies as $movie)
                            <div class="movie-card-wrapper" id="mb_0">
                                <div class="movie-card">
                                    ...
                                    <div class="movie-content">
                                        ...
                                        <button type="button" class="btn btn-light"><i
                                                ...
                                                href="{{ route('movieDetailsPage', [$movie['slug'], $movie['type']]) }}">
                                                Watch </a></button>
