                            ...
                            <div class="card card-primary">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <input type="text" hidden name="rowId" id="rowId" class="rowId" value="{{@$slider->id}}">
                                        <input type="text" name="transcodeStatus" value="{{@$slider->transcodeStatus=='true' ? 'true':'false'}}" hidden id="transcodeStatus">
                                        <div class="col-6 mb-2">
                                            {{ Form::label('status', 'Status:', ['class' => 'form-label']) }}
                                            {{ Form::select('status', ['active' => 'Active', 'inactive' => 'In-Active'], @$slider->status, ['class' => 'form-control  ' . ($errors->has('status') ? 'is-invalid' : ''), 'placeholder' => '------------Select If Any---------------', 'required' => true]) }}
                                        ...
                                        <div class="col-6 mb-2">
                                            ...
                                            <select name="type" id="type" class="form-control" required>
                                                ...
                                                <option value="file" {{ @$slider->type=='file' ? 'selected' : ''}}>Image</option>
                                        ...
                                        <label for="item_type" class="form-label">Type</label>
                                            <select name="item_type" id="item_type" class="form-control" >
                                                ...
                                                @foreach ($itemTypes as $type)
                                                    <option value="{{$type->value}}" {{ @$type->value==@$slider->item_type->value ? 'selected' : ''}}>{{$type->name}}</option>
                                                @endforeach
                                            </select>
                                            <span class="text-danger" id="typeError" hidden></span>
                                        </div>
                                            ...
                                            <div class="row justify-content-center" style="margin-right: 40%;display:block">
                                                <div class="col-md-8">
                                                        <div class="card-footer p-4" style="display: block">
                                                            <video id="videoPreview" src="{{$slider->path}}" controls
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('change','#item_type',function(){
            let itemTypeId=$(this).val() ?? "{{@$slider->item_type->value}}";
            let movieId="{{@$slider->movie_id}}" ?? 0;
            if(!itemTypeId){
                $('#movieId').html('');
        ...
        $(document).on('click','#submitForm',function(){
            ...
            let itemType=null;
            let movie_id=null;
            if(formData['item_type'] && formData['item_type'].value){
                itemType = formData['item_type'].value;
            }else{
                 itemType=null;
            }
            if(formData['movie_id'] && formData['movie_id'].value){
                 movie_id=formData['movie_id'].value;
            }else{
                 movie_id=null;
            }
            if(formData['path']){
            ...
            $.ajax({
                ...
                data:{
                    ...
                    checkVideoFile:checkVideoFile,
                    item_type:itemType,
                    movie_id:movie_id,
                    transcodeStatus:formData['transcodeStatus'].value
                },
                success:function(response){
