        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <a href="{{route('post.create')}}" class="btn btn-sm btn-success ml-auto">Add Post</a>
                </div>
            </div><!-- /.container-fluid -->
        </section>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($posts as $key=>$data)
                                            <tr>
                                                ...
                                                <td>{{ $data->title }}</td>
                                                <td>
                                                    @foreach ($data->postCategory->pluck('name') as $cat)
                                                    <span class="badge badge-success">{{$cat}}</span>
                                                    @endforeach
                                                </td>
                                                <td>{{$data->status->name}}</td>
                                                <td>
                                                    <a class="btn btn-sm btn-success" href="{{route('post.edit',$data->id)}}">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="javascript:;" class="btn btn-sm  icon btn-rounded btn-danger btn-style delete-visitor" data-id="{{ $data->id}}">
                                                        <i class="fas fa-trash"></i>
                                                    </a>
                                                    {{ Form::open(['url'=>route('post.destroy',$data->id),'class'=>'delete-form'])}}
                                                        @method('delete')
                                                    {{ Form::close()}}
