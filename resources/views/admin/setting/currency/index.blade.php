                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="card card-primary">
                            ...
                            <div class="card-body">
                                {{Form::open(['url'=>route('currency.store'),'class'=>'form form-horizontal'])}}
                                <div class="row">
                                    ...
                                    <div class="col-md-12 form-group mt-2">
                                        ...
                                        <input type="text" min="1" name="value" id="value" class="form-control" value="{{old('value',@$seoSocial->value)}}" required>
                                        @error('value')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <tbody>
                                        @foreach($currencies as $key=>$data)
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-sm btn-success" href="{{route('currency.edit',$data->id)}}">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="javascript:;" class="btn btn-sm  icon btn-rounded btn-danger btn-style delete-visitor" data-id="{{ $data->id}}">
                                                        <i class="fas fa-trash"></i>
                                                    </a>
                                                    {{ Form::open(['url'=>route('currency.destroy',$data->id),'class'=>'delete-form'])}}
                                                        @method('delete')
                                                    {{ Form::close()}}
...
@push('scripts')
    <script>
        ...
        e.preventDefault();
        let clicked=confirm('Are You Sure Want To Delete Currency');
