            ...
            <div class="row">
                <div class="card">
                    <div class="card-body">
                        ...
                        @isset($permission)
                        {{ Form::open(['url' => route('permission.update', $permission->id), 'files' => true, 'class' => 'form form-horizontal']) }}
                        @method('put')
                    @else
                        {{ Form::open(['url' => route('permission.store'), 'files' => true, 'class' => 'form form-horizontal']) }}
                    @endisset

                            @csrf
                            <div class="col-12">
                                <label class="form-label" for="modalRoleName">Permission Name</label>
                                <input type="text" id="modalRoleName" name="name" value="{{ @$permission->name }}"
