@extends('admin.app')
@section('title', 'Cinemas Branch List')
@section('main')
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <a href="{{route('cinemasbranch.create')}}" class="btn btn-sm btn-success ml-auto">Create Cinemas Branch</a>
                </div>
            </div><!-- /.container-fluid -->
        </section>
                                    ...
                                    <tbody>
                                        @foreach($notifications as $key=>$data)
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-sm btn-success" href="{{route('cinemasbranch.edit',$data->id)}}">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="javascript:;" class="btn btn-sm  icon btn-rounded btn-danger btn-style delete-visitor" data-id="{{ $data->id}}">
                                                        <i class="fas fa-trash"></i>
                                                    </a>
                                                    {{ Form::open(['url'=>route('cinemasbranch.destroy',$data->id),'class'=>'delete-form'])}}
                                                        @method('delete')
                                                    {{ Form::close()}}
